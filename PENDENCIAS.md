...
- **Redesign da landing v2 (fio dourado + cinematográfico)** — concluído e commitado na `staging` (`851603b`). Bug do scroll travado corrigido (ScrollTrigger pinava via `position:fixed`, quebrado por `overflow-x:hidden` no ancestral — trocado pra `pinType:"transform"`). Build+lint limpos. **Falta só o usuário testar visualmente** (é a terceira tentativa de landing — as duas anteriores foram rejeitadas) antes de considerar fechado e pensar em merge pra `main`.
