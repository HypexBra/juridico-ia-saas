...
import { recuperarSenhaPortalAction, type RecuperarSenhaState } from "./actions";
import { Button } from "@/components/ui/button";
import { Input, Label } from "@/components/ui/input";

const INITIAL_STATE: RecuperarSenhaState = { enviado: false, error: null };

export function RecuperarSenhaPortalForm() {
  const [state, formAction, isPending] = useActionState(recuperarSenhaPortalAction, INITIAL_STATE);
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <p className="text-center text-sm text-muted">
        <Link href="/portal/login" className="font-medium text-silver underline underline-offset-2 hover:text-silver-2">
          Voltar para o login
        </Link>
      </p>
    </form>
