...
export default async function PortalLayout({ children }: LayoutProps<"/portal">) {
  ...
  return (
    <div className="min-h-screen bg-navy">
      <header className="sticky top-0 z-30 border-b border-ink/10 bg-navy/95 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-3xl items-center justify-between gap-3 px-4 sm:px-6">
          ...
          >
            ...
            <span className="hidden rounded-full border border-ink/10 bg-ink/5 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-muted sm:inline">
              Portal do Cliente
            </span>
          </Link>
          ...
          {clientePortalAtual && (
            <form action={portalLogoutAction}>
              <button
                type="submit"
                className="cursor-pointer rounded-lg px-3 py-2 text-sm font-medium text-muted transition-colors hover:bg-ink/5 hover:text-ice"
              >
                Sair
              </button>
            </form>
