...
export function AdicionarAdminForm() {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      {state && (
        <p className={`text-xs sm:ml-3 ${state.ok ? "text-muted" : "text-red-400"}`}>{state.ok ? state.mensagem : state.error}</p>
      )}
    </form>
  );
}
  ...
  return (
    <tr>
      ...
      <td className="py-3 pr-3">
        <span className={admin.ativo ? "text-green" : "text-red-400"}>{admin.ativo ? "Ativo" : "Inativo"}</span>
      </td>
      <td className="py-3">
          ...
          <div className="flex justify-end gap-1.5">
            <button
              ...
              className={`rounded-md border px-2 py-1 text-xs disabled:opacity-50 ${
                admin.ativo ? "border-red-500/30 text-red-300 hover:bg-red-500/10" : "border-green/30 text-green hover:bg-green/10"
              }`}
            >
